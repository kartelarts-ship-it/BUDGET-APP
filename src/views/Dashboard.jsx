// ─── KACORP BUDGET V5 · views/Dashboard.jsx ─────────────────────

import { useState, useRef, useEffect } from "react";
import { C } from "../constants.js";
import {
  Card, Lbl, GhostBtn, Ring, Bar, Chip, StatRow,
...
export function Dashboard({ state, set, inc, totalExp, f }) {
  ...
  const totalVersedMonth = state.goals.reduce(
    (s, g) => s + versedThisMonth(g), 0
  );
  const totalPlanned = activeGoals.reduce(
    (s, g) => s + (g.selectedPlan ? parseAmt(g.selectedPlan.monthly) : 0), 0
  );
  const resteAVerser = Math.max(totalPlanned - totalVersedMonth, 0);
  const resteLibre   = capaciteEpargne - totalAlloc;

  const badgeLabel  = realSaveRate >= 20 ? "EXCELLENT" : realSaveRate >= 10 ? "BON" :
                      realSaveRate > 0  ? "À AMÉLIORER" : "NON MESURÉ";
  const badgeColor  = realSaveRate >= 20 ? C.pos : realSaveRate >= 10 ? "#34D399" :
                      realSaveRate > 0  ? C.warn : C.muted;

  const planStatus = resteLibre > inc * 0.15 ? "confortable"
    : resteLibre > 0 ? "équilibré"
    : resteLibre > -inc * 0.1 ? "serré" : "insuffisant";
  const planColor = planStatus === "confortable" ? C.pos
    ...
    <div className="screen" style={{ padding:"18px 14px", display:"flex",
      ...
      <Card>
        ...
        {editIncome ? (
          ...
          <div onClick={() => setEditIncome(true)} style={{ cursor:"pointer" }}>
            ...
            <div style={{ fontSize:11, color:C.label, marginTop:4 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
        ...
        <Card style={{ display:"flex", flexDirection:"column",
          ...
          <Ring value={Math.max(realSaveRate, 0)} max={100}
            color={badgeColor} size={72} sw={6}>
        ...
        <Card style={{ display:"flex", flexDirection:"column", gap:8 }}>
          ...
          <div style={{ fontSize:11, color:C.label }}>
            soit {f(inc * recRate / 100)} / mois
          </div>
          <Bar value={totalVersed} max={inc * recRate / 100}
      ...
      <Card style={{ background:C.s2 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:0 }}>
          ...
          <StatRow label="Total prévu (objectifs)" value={f(totalPlanned)} color={C.text}/>
      ...
      {/* ── Actions du mois (par objectif) ── */}
      {activeGoals.length > 0 && (
        <Card>
          <Lbl style={{ marginBottom:12 }}>ACTIONS DU MOIS</Lbl>
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            {activeGoals.map(g => {
              const planned   = g.selectedPlan ? parseAmt(g.selectedPlan.monthly) : null;
              const versed    = versedThisMonth(g);
              const status    = computeGoalStatus(g);
              const sCol      = STATUS_COLORS[status];
              const toVerse   = planned != null ? Math.max(planned - versed, 0) : null;
              const deadline  = g.selectedPlan?.date || "—";
              return (
                <div key={g.id} style={{ display:"flex", alignItems:"center",
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ fontSize:11, color:C.label }}>
                      {planned != null
                        ? (toVerse > 0 ? "À verser : " + f(toVerse) : "Versé ✓") +
                          " · " + deadline
                        : "Aucun plan défini"}
                    </div>
                  </div>
                  <Chip label={status.toUpperCase()} color={sCol}/>
                </div>
              );
            })}
          </div>
